...
import { J as xe } from './JupiterService-DgkMwrEN.js';
import { g as He, a as ae, c as et } from './transactions-CiIB2wJI.js';
import { a as ie } from './types-BdLee-_x.js';
import { C as z } from './CapacityLimitIcon-CWXOclGs.js';
import { t as tt, a as st } from './tryAnchorErrorMessageOr-DZlonrEi.js';
import { u as nt } from './useSignAllTransactions-DXWp4UND.js';
import { u as rt } from './useBudgetAndPriorityFeeIxns-maDZLxcK.js';
import { u as ot } from './useModal-0llzXuOc.js';
import { u as le } from './useJupiterPriceQuery-CWRiqL8a.js';
import { c as at, b as it, a as ce } from './useGetLSTsStakeRatesQuery-By0XPz62.js';
import { u as lt } from './useKswapSdk-6qh4k4RZ.js';
(function () {
  try {
    var e =
        typeof window < 'u'
    ...
    t &&
      ...
      (e._sentryDebugIds[t] = '480368e4-6869-4069-a192-718bc0023516'),
      (e._sentryDebugIdIdentifier = 'sentry-dbid-480368e4-6869-4069-a192-718bc0023516'));
  } catch (o) {}
})();
function Vt(e, t, o) {
...
const $t = async (e, t) => {
  ...
  Xt = () => {
    ...
    return !e || !t || e === t || !Pe.isReferralPageEnabled
...
function Jt() {
  ...
  return {
    isWalletConnect: P.useMemo(() => (!t || !e ? !1 : t.adapter.name === 'WalletConnect'), [t, e]),
  };
}
const ut = '_root_1k7i4_1',
  ...
  vt = () => {
    ...
    return {
      isReserveDisabledToBeUseAsCollOutsideOfElevationGroup: P.useCallback((a) => t.get(a), [t]),
    };
  },
  Ht = ({ reserve: e }) => {
    const { getMarketByAddress: t } = _(),
      { getReserveByAddress: o } = W(),
      { data: a } = K({});
    return P.useMemo(
      () => ({
  ...
  xt = ({ reserveAddress: e, marketAddress: t }) => {
    const { getMarketByAddress: o } = _(),
      ...
      d = P.useCallback((r) => c.find((x) => x.id === r.toString() && x.id !== '0'), [c]);
    return {
      data: c,
  ...
  ts = ({
    ...
    const { activeReserves: i, getReserveByAddress: s } = W(),
      { getToken: u } = F(),
      { data: c } = xt({
        reserveAddress: t,
      ...
      w = P.useCallback(
        (y) => {
          const b = [],
            ...
            B = Array.from((e == null ? void 0 : e.deposits.values()) || []).map((k) =>
              k.reserveAddress.toString()
            ),
            q = j.reduce((k, J) => {
              if (B.includes(J.address.toString())) {
  ...
  It = ({ onSuccess: e = re, onCloseSetupModal: t = re }) => {
    const { walletPublicKey: o, wallet: a } = Q(),
      n = Z(),
      { addNotification: i, updateTxByIndex: s } = We(),
...
function os({
  ...
  return {
    handleSetupAccount: () => {
      c({
        txsIxns: a,
        queryKey: G.getUserLutAndSetupIxns(n != null ? n : '', e, t || '', o || ''),
      }),
        u();
    },
  ...
  const h = async (w, S) => {
    ...
    f = await (async () => {
      ...
      switch (g.formType) {
        ...
        case 'withdraw': {
          if (!r) throw new Error('No obligation found. Please, refresh the page and try again');
          const { amount: A, selectedTokenMint: E, deposited: j, borrowed: B, maxAmount: q } = g;
...
function is({
  ...
  return Re({
    queryKey: G.getPriceImpactRequiredData({
      marketAddress: e,
      debtTokenMint: a,
      collTokenMint: n,
      leverage: i,
...
//# sourceMappingURL=AccountSetupModal-BuWPVIbK.js.map
