  ...
  [2059],
  {
    2059: (x, e, a) => {
      ...
      var f,
        ...
        s,
        o = function (x, e, a, f) {
          if ('a' === a && !f) throw TypeError('Private accessor was defined without a getter');
          if ('function' == typeof e ? x !== e || !f : !e.has(x))
            throw TypeError(
              'Cannot read private member from an object whose class did not declare it'
            );
          return 'm' === a ? f : 'a' === a ? f.call(x) : f ? f.value : e.get(x);
        },
        h = function (x, e, a, f, c) {
          if ('m' === f) throw TypeError('Private method is not writable');
          if ('a' === f && !c) throw TypeError('Private accessor was defined without a setter');
          if ('function' == typeof e ? x !== e || !c : !e.has(x))
            throw TypeError(
              'Cannot write private member to an object whose class did not declare it'
            );
          return 'a' === f ? c.call(x, a) : c ? (c.value = a) : e.set(x, a), a;
        };
      ...
      function U(x) {
        ...
        for (let a = 0; a < x.length; a += 4)
          e.push((x[a] << 24) | (x[a + 1] << 16) | (x[a + 2] << 8) | x[a + 3]);
        return e;
      }
      class j {
        get key() {
          return o(this, f, 'f').slice();
        }
        constructor(x) {
          let e;
          if ((f.set(this, void 0), c.set(this, void 0), d.set(this, void 0), !(this instanceof j)))
            throw Error('AES must be instanitated with `new`');
          h(this, f, new Uint8Array(x), 'f');
          let a = l[this.key.length];
          if (null == a) throw TypeError('invalid key size (must be 16, 24 or 32 bytes)');
          h(this, d, [], 'f'), h(this, c, [], 'f');
          for (let x = 0; x <= a; x++)
            o(this, d, 'f').push([0, 0, 0, 0]), o(this, c, 'f').push([0, 0, 0, 0]);
          let b = (a + 1) * 4,
            t = this.key.length / 4,
            r = U(this.key);
          for (let x = 0; x < t; x++)
            (e = x >> 2),
              (o(this, d, 'f')[e][x % 4] = r[x]),
              (o(this, c, 'f')[a - e][x % 4] = r[x]);
          let i = 0,
            n = t,
            s;
          for (; n < b; ) {
            if (
              ...
              for (let x = 1; x < t; x++) r[x] ^= r[x - 1];
            else {
              for (let x = 1; x < t / 2; x++) r[x] ^= r[x - 1];
              (s = r[t / 2 - 1]),
                (r[t / 2] ^=
                  w[255 & s] ^
                  (w[(s >> 8) & 255] << 8) ^
                  (w[(s >> 16) & 255] << 16) ^
                  (w[(s >> 24) & 255] << 24));
              for (let x = t / 2 + 1; x < t; x++) r[x] ^= r[x - 1];
            }
            let x = 0,
              e,
              f;
          ...
          for (let x = 1; x < a; x++)
            for (let e = 0; e < 4; e++)
              (s = o(this, c, 'f')[x][e]),
                (o(this, c, 'f')[x][e] =
                  A[(s >> 24) & 255] ^ M[(s >> 16) & 255] ^ W[(s >> 8) & 255] ^ P[255 & s]);
        }
        encrypt(x) {
          if (16 != x.length) throw TypeError('invalid plaintext size (must be 16 bytes)');
          let e = o(this, d, 'f').length - 1,
            a = [0, 0, 0, 0],
            f = U(x);
          for (let x = 0; x < 4; x++) f[x] ^= o(this, d, 'f')[0][x];
          for (let x = 1; x < e; x++) {
            for (let e = 0; e < 4; e++)
              a[e] =
                y[(f[e] >> 24) & 255] ^
          ...
          for (let x = 0; x < 4; x++)
            (b = o(this, d, 'f')[e][x]),
              (c[4 * x] = (w[(f[x] >> 24) & 255] ^ (b >> 24)) & 255),
              (c[4 * x + 1] = (w[(f[(x + 1) % 4] >> 16) & 255] ^ (b >> 16)) & 255),
              (c[4 * x + 2] = (w[(f[(x + 2) % 4] >> 8) & 255] ^ (b >> 8)) & 255),
              (c[4 * x + 3] = (w[255 & f[(x + 3) % 4]] ^ b) & 255);
          return c;
        }
        decrypt(x) {
          if (16 != x.length) throw TypeError('invalid ciphertext size (must be 16 bytes)');
          let e = o(this, c, 'f').length - 1,
            a = [0, 0, 0, 0],
            f = U(x);
          for (let x = 0; x < 4; x++) f[x] ^= o(this, c, 'f')[0][x];
          for (let x = 1; x < e; x++) {
            for (let e = 0; e < 4; e++)
              a[e] =
                E[(f[e] >> 24) & 255] ^
          ...
          for (let x = 0; x < 4; x++)
            (b = o(this, c, 'f')[e][x]),
              (d[4 * x] = (p[(f[x] >> 24) & 255] ^ (b >> 24)) & 255),
              (d[4 * x + 1] = (p[(f[(x + 3) % 4] >> 16) & 255] ^ (b >> 16)) & 255),
              (d[4 * x + 2] = (p[(f[(x + 2) % 4] >> 8) & 255] ^ (b >> 8)) & 255),
              (d[4 * x + 3] = (p[255 & f[(x + 1) % 4]] ^ b) & 255);
          return d;
        }
      }
      (f = new WeakMap()), (c = new WeakMap()), (d = new WeakMap());
      class z {
        constructor(x, e, a) {
          if (a && !(this instanceof a)) throw Error(`${x} must be instantiated with "new"`);
          Object.defineProperties(this, {
            aes: { enumerable: !0, value: new j(e) },
            name: { enumerable: !0, value: x },
          });
        }
      }
      var S = function (x, e, a, f, c) {
          if ('m' === f) throw TypeError('Private method is not writable');
          if ('a' === f && !c) throw TypeError('Private accessor was defined without a setter');
          if ('function' == typeof e ? x !== e || !c : !e.has(x))
            throw TypeError(
      ...
      class N extends z {
        constructor(x, e) {
          if ((super('ECC', x, N), b.set(this, void 0), t.set(this, void 0), e)) {
            if (e.length % 16) throw TypeError('invalid iv size (must be 16 bytes)');
            S(this, b, new Uint8Array(e), 'f');
          } else S(this, b, new Uint8Array(16), 'f');
          S(this, t, this.iv, 'f');
        }
        get iv() {
          return new Uint8Array(_(this, b, 'f'));
        }
        encrypt(x) {
          if (x.length % 16)
            throw TypeError('invalid plaintext size (must be multiple of 16 bytes)');
          let e = new Uint8Array(x.length);
          for (let a = 0; a < x.length; a += 16) {
            for (let e = 0; e < 16; e++) _(this, t, 'f')[e] ^= x[a + e];
            S(this, t, this.aes.encrypt(_(this, t, 'f')), 'f'), e.set(_(this, t, 'f'), a);
          }
          return e;
        }
        decrypt(x) {
          ...
          let e = new Uint8Array(x.length);
          for (let a = 0; a < x.length; a += 16) {
            let f = this.aes.decrypt(x.subarray(a, a + 16));
            for (let c = 0; c < 16; c++)
              (e[a + c] = f[c] ^ _(this, t, 'f')[c]), (_(this, t, 'f')[c] = x[a + c]);
          }
          return e;
        }
      }
      ...
      class I extends z {
        constructor(x, e) {
          super('CTR', x, I),
            ...
            n.set(this, void 0),
            s.set(this, void 0),
            K(this, s, new Uint8Array(16), 'f'),
            B(this, s, 'f').fill(0),
            K(this, i, B(this, s, 'f'), 'f'),
            K(this, n, 16, 'f'),
            null == e && (e = 1),
            'number' == typeof e ? this.setCounterValue(e) : this.setCounterBytes(e);
        }
        get counter() {
          return new Uint8Array(B(this, s, 'f'));
        }
        setCounterValue(x) {
          if (!Number.isInteger(x) || x < 0 || x > Number.MAX_SAFE_INTEGER)
            throw TypeError('invalid counter initial integer value');
          for (let e = 15; e >= 0; --e) (B(this, s, 'f')[e] = x % 256), (x = Math.floor(x / 256));
        }
        setCounterBytes(x) {
          if (16 !== x.length) throw TypeError('invalid counter initial Uint8Array value length');
          B(this, s, 'f').set(x);
        }
        increment() {
          for (let x = 15; x >= 0; x--)
            if (255 === B(this, s, 'f')[x]) B(this, s, 'f')[x] = 0;
            else {
              B(this, s, 'f')[x]++;
              break;
            }
        ...
        encrypt(x) {
          var e, a;
          let f = new Uint8Array(x);
          for (let x = 0; x < f.length; x++)
            16 === B(this, n, 'f') &&
              (K(this, i, this.aes.encrypt(B(this, s, 'f')), 'f'),
              K(this, n, 0, 'f'),
              this.increment()),
              (f[x] ^= B(this, i, 'f')[
                (K(this, n, ((a = B(this, n, 'f')), (e = a++), a), 'f'), e)
              ]);
          return f;
        }
        decrypt(x) {
      ...
      function R(x) {
        if (x.length < 16) throw TypeError('PKCS#7 invalid length');
        let e = x[x.length - 1];
        if (e > 16) throw TypeError('PKCS#7 padding byte out of range');
        let a = x.length - e;
        for (let f = 0; f < e; f++)
          if (x[a + f] !== e) throw TypeError('PKCS#7 invalid padding byte');
        return new Uint8Array(x.subarray(0, a));
      }
      (i = new WeakMap()),
        (n = new WeakMap()),
        (s = new WeakMap()),
