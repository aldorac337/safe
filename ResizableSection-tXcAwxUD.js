...
var O = (s, e, t) =>
  ...
  P = (s, e) => te(s, se(e));
import { r as y, j as a, ao as oe, t as ae, c as F, w as ie } from './vendor-S4AjIOSE.js';
import { f as B, D as U, C as ce, h as Y, j as le, p as de, Z as he } from './index-CroKpwmf.js';
import { D as ue } from './charting_library.esm-Bio8dT3Z.js';
(function () {
  try {
    var s =
        typeof window < 'u'
    ...
    e &&
      ...
      (s._sentryDebugIds[e] = 'b361c3e7-6ec5-4a5e-b3b8-2f41982b6e31'),
      (s._sentryDebugIdIdentifier = 'sentry-dbid-b361c3e7-6ec5-4a5e-b3b8-2f41982b6e31'));
  } catch (t) {}
})();
  ...
  if (_ && h) {
    ...
    if (m === e) {
      ...
      o >= l
        ...
        (_.lastDailyBar = g),
        (v = _.handlers) == null || v.forEach((p) => p.callback(g));
    }
    if (m === t) {
      const l = W(b.time);
      o >= l
        ? (g = { time: l, open: n, high: Math.max(b.high, n), low: Math.min(b.low, n), close: n })
        : (g = P($({}, b), { high: n, low: n, close: n })),
        (h.lastDailyBar = g);
    }
    T.set(e, _), T.set(t, h);
  }
...
function Ke(s, e, t, r, i, c, n, o) {
  ...
  if (m && _) {
    const f = { id: i, callback: r },
      b = T.get(m) || { handlers: [], resolution: t, lastDailyBar: n };
    b.handlers && (b.handlers = (h = b.handlers) == null ? void 0 : h.filter((l) => l.id !== i)),
      (v = b.handlers) == null || v.push(f),
      (b.resolution = t),
      T.set(m, b);
...
const R = 'https://benchmarks.pyth.network/v1/shims/tradingview',
  ...
  yt = {
    ...
    getBars: async (s, e, t, r, i) => {
      ...
      let g = Math.max(c, f),
        l;
      const p = [];
      for (; g < n; ) (l = Math.min(n, g + h)), p.push({ from: g, to: l }), (g = l);
      const u = [],
        S = [],
        D = p.map((d) =>
          fetch(
  ...
  bt = ({
    ...
    const n = y.useRef(),
      ...
      b = y.useCallback(() => {
        var p;
        const l = (p = o.current) == null ? void 0 : p.activeChart();
        if (l) {
          ...
          try {
            if ((V(h), i.length > 0)) {
              const l = o.current.activeChart(),
                p = i.filter((u) => u.forceLineInViewOnMainPane);
              ...
              if (p.length > 0) {
                ...
                if (u) {
                  ...
                  if (S) {
                    const D = p.filter((j) => j.value >= S.from * 0.5 && j.value <= S.to * 1.5);
                    if (D.length > 0) {
    ...
    return (
      y.useEffect(() => {
        ...
        return (
          ...
          () => {
            _(!1), V(h), p.remove(), (o.current = null);
          }
        );
      }, [n, t, f, c, s, e]),
      y.useEffect(() => {
  ...
  pt = ({ children: s, startHeight: e }) => {
    const [t, r] = y.useState(e),
      ...
      f = (g) => {
        if (o.current && n.current) {
          const p = g.touches[0].clientY - n.current.getBoundingClientRect().top;
          p > e && r(p), g.preventDefault();
        }
      },
      b = () => {
        (o.current = !1),
...
export { pt as R, gt as S, bt as T, ft as a, Je as b, yt as d, _t as u };
//# sourceMappingURL=ResizableSection-tXcAwxUD.js.map
